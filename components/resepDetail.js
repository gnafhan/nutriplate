...
import recipeData from "../db/resep";
...
const RecipePage = ({ recipeId }) => {
  ...
  return (
    <div className="max-w-[1030px] mx-auto px-4 sm:px-8 xl:px-0">
      {/* Header Section */}
      <div
        className={`max-w-[770px] mx-auto text-center transform transition-all duration-700 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}
      >
        <a
          href="/resep-makanan"
      ...
      {/* Content Section */}
      <div
        className={`max-w-[770px] mx-auto bg-white rounded-xl p-6 shadow-sm transition-all duration-700 delay-200 ${isVisible ? "opacity-100" : "opacity-0"}`}
      >
        <div>
          {/* Ingredients Section */}
          <div
            className={`transition-all duration-700 delay-300 transform ${isVisible ? "translate-x-0 opacity-100" : "-translate-x-4 opacity-0"}`}
          >
          ...
          {/* Steps Section */}
          <div
            className={`transition-all duration-700 delay-400 transform ${isVisible ? "translate-x-0 opacity-100" : "translate-x-4 opacity-0"}`}
          >
            <h2 className="my-5 text-2xl font-semibold text-primary">
              Cara pembuatan
            </h2>
